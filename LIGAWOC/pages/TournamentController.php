...
class TournamentController {
    ...
    public function createTournament($data) {
        return $this->db->insert(
            "INSERT INTO tournaments (name, slug, description, rules, image, season_id, format, team_size, max_teams, min_rank, entry_fee, prize_pool, status, registration_start, registration_end, start_date, end_date, is_main_tournament, tournament_type, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [
                $data['name'], slugify($data['name']), $data['description'] ?? '', $data['rules'] ?? '',
                $data['image'] ?? null, $data['season_id'] ?? null, $data['format'],
                $data['team_size'] ?? 5, $data['max_teams'] ?? 16, $data['min_rank'] ?? null,
    ...
    public function updateTournament($id, $data) {
        $sql = "UPDATE tournaments SET name = ?, description = ?, rules = ?, format = ?, team_size = ?, max_teams = ?, min_rank = ?, entry_fee = ?, prize_pool = ?, status = ?, registration_start = ?, registration_end = ?, start_date = ?, end_date = ?, stream_url = ?, is_main_tournament = ?, season_id = ?";
        $params = [
    ...
    public function registerTeam($tournamentId) {
        ...
        if (count($members) < $tournament['team_size']) {
            setFlash('error', 'Tu equipo necesita al menos ' . $tournament['team_size'] . ' miembros para inscribirse.');
            redirect('tournaments/view/' . $tournamentId);
        }

        // Roster validation
        $rosterIds = $_POST['roster'] ?? [];
        if (!is_array($rosterIds) || count($rosterIds) !== (int)$tournament['team_size']) {
            setFlash('error', 'Debes seleccionar exactamente ' . $tournament['team_size'] . ' jugadores para el roster.');
            redirect('tournaments/view/' . $tournamentId);
        }
        ...
        foreach ($members as $m) {
            if (in_array($m['user_id'], $rosterIds)) {
                $validRoster[] = (int)$m['user_id'];
            }
        }
        
        if (count($validRoster) !== (int)$tournament['team_size']) {
        ...
        $this->db->insert(
            "INSERT INTO tournament_teams (tournament_id, team_id, roster, status, payment_status) VALUES (?, ?, ?, 'registered', ?)",
            [$tournamentId, $teamId, $rosterJson, $paymentStatus]
        );
    ...
    private function generateSingleElimination($tournamentId, $teams) {
        ...
        for ($i = 0; $i < $bracketSize; $i += 2) {
            ...
            if ($team1 && !$team2) {
                $this->db->update(
                    "UPDATE tournament_matches SET winner_id = ?, status = 'completed' WHERE tournament_id = ? AND match_number = ? AND round = 1",
                    [$team1['team_id'], $tournamentId, $matchNumber]
                );
            }
    ...
    public function getRegisteredTeams($tournamentId) {
        ...
        // If the viewer has correct privileges, attach the 5-man roster data
        if (isAdmin() || isSuperAdmin() || isModerator()) {
            foreach ($teams as &$t) {
                $t['roster_data'] = [];
                if (!empty($t['roster'])) {
                    $playerIds = json_decode($t['roster'], true);
                    if (is_array($playerIds) && count($playerIds) > 0) {
                        $placeholders = implode(',', array_fill(0, count($playerIds), '?'));
                        $query = "SELECT id, username, email, ml_nickname, whatsapp, phone_brand, discord FROM users WHERE id IN ($placeholders)";
                        $players = $this->db->fetchAll($query, $playerIds);
